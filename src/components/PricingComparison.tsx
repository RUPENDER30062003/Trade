...
const PricingComparison = () => {
  ...
  // Animated border styles for all plans
    ...
    <div className="w-full bg-[#050507] text-white py-8 md:py-16">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center mb-12 relative">
        ...
        <div className="relative overflow-visible">
          ...
          {/* Header content with z-index to appear above the glowing border */}
          <div className="relative z-10 py-10 px-6 bg-[#111116] rounded-xl border border-purple-500/30">
            ...
            >
              Compare your <span className="text-purple-400 font-medium">Abcd</span>{" "}
              plan
            </motion.h2>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="col-span-1">
            <div className="h-28 flex items-end mb-6">
              {/* Empty header space to align with plan cards */}
            </div>
            ...
            {features.map((feature, index) => (
              <motion.div
                ...
                custom={index + 3}
                variants={fadeIn}
                className="py-4 px-4 border-t border-gray-800 flex items-center h-20"
              >
          ...
          {plans.map((plan, planIndex) => (
            ...
            >
              {/* Entire column glowing border effect */}
              <div className="relative overflow-visible h-full">
                <motion.div
                  ...
                  variants={glowingBorderVariants}
                  className="absolute inset-0 rounded-xl"
                  style={{ height: "calc(100% + 40px)" }} // Reduced height extension
                >
                  <svg className="absolute top-0 left-0 w-full h-full">
                    <motion.rect
                      x="0"
                ...
                <div className="relative z-10 bg-[#111116] rounded-xl p-4 border border-purple-500/30">
                  ...
                  {features.map((feature, index) => (
                    <motion.div
                      ...
                      initial="hidden"
                      animate="visible"
                      custom={index + planIndex + 3}
                      variants={fadeIn}
                      className={`py-4 px-4 border-t border-gray-800 h-18 flex items-center justify-center text-center
                        ${
                          feature.key === "riskExposure"
                            ? plan[feature.key] === "Low"
                              ? "text-green-400"
                              : plan[feature.key] === "Moderate"
                              ? "text-yellow-400"
                              : "text-red-400"
                            : "text-white"
                        }
                        ${feature.key === "description" ? "text-sm" : "text-base"} text-gray-700 mb-2
                      `}
                    >
                      {plan[feature.key]}
                    </motion.div>
                  ...
                  <div className="mt-4 flex justify-center mb-2">
                    <button
                      className="w-full bg-purple-600 hover:bg-purple-700 text-white py-4 px-8 rounded-full transition duration-300"
                    >
                      Start Trading
                    </button>
                  </div>
